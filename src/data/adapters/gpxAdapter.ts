...
export class GPXAdapter extends BaseAdapter {
    readonly id = 'gpx';
    readonly supportedExtensions = ['.gpx'];

    async parse(fileUri: { fsPath: string }): Promise<ParsedForKepler> {
        const content = await fs.readFile(fileUri.fsPath, 'utf-8');
        
        try {
            const parsed = await parse(content, GPXLoader) as any;
            ...
            throw new Error(`Failed to parse GPX: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
